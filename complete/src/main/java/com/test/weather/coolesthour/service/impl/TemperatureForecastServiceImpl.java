...
import com.test.weather.coolesthour.exceptions.SetupException;
import com.test.weather.coolesthour.exceptions.TemperatureNotFoundException;
import com.test.weather.coolesthour.integrations.response.model.ResponseCodeAndBody;
import com.test.weather.coolesthour.integrations.response.model.TemperatureData;
import com.test.weather.coolesthour.integrations.response.model.TemperatureResponse;
        ...
        try {
            ...
            for (TemperatureData data : temperatureResponse.getHourly().getData()) {
                ...
                if (date.getDayOfYear() == tomorrow.getDayOfYear()) {
                    temperatures.add(new TemperatureAtTime(data.getTemperature(), date,0));
                }
            }
